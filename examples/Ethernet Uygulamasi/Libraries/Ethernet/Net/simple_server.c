...
u16 print_webpage(u8* buf, u8 on_off)
{
   ...
   plen = fill_tcp_data_p(buf, plen, PSTR("<center><p>LED Durumu: "));
   if (on_off)
   {
      plen = fill_tcp_data_p(buf, plen, PSTR("<font color=\"#FF0000\"> LED Açýk</font>"));
   }
   else
   {
      plen = fill_tcp_data_p(buf, plen, PSTR("LED Kapalý"));
   }
   plen = fill_tcp_data_p(buf, plen, PSTR(" <body bgcolor=\"#3366FF\"> "));
   plen = fill_tcp_data_p(buf, plen, PSTR(" <small><a href=\""));
   plen = fill_tcp_data(buf, plen, baseurl);
   plen = fill_tcp_data(buf, plen, password);
   plen = fill_tcp_data_p(buf, plen, PSTR("\"></a></small></p>\n<p><a href=\""));
   // the url looks like this http://baseurl/password/command
   plen = fill_tcp_data(buf, plen, baseurl);
   plen = fill_tcp_data(buf, plen, password);
   if (on_off)
   {
      plen = fill_tcp_data_p(buf, plen, PSTR("/0\">LED Kapat</a><p>"));
   }
   else
   {
      plen = fill_tcp_data_p(buf, plen, PSTR("/1\">LED Ac</a><p>"));
   }
   plen = fill_tcp_data_p(buf, plen, PSTR("Expkits - ElektroEmre - Ethernet Test</a><p>"));
